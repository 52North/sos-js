        ...
        "gml": OpenLayers.Util.applyDefaults({
            ...
            "pos": function(node, obj) {
                ...
                if (!this.externalProjection) {
                  /* Modified by Paul M. Breen, 2012-09-27.  Required for
                   * using OL with EPSG:3031.  It's critical that we only try
                   * to reproject if the response from the SOS actually
                   * contains an srsName attribute, otherwise it causes proj4js
                   * to fail with an error, which ultimately causes the whole
                   * script to fail.  The SOS will return a valid response
                   * without an srsName attribute when, for instance, the given
                   * FOI doesn't relate to the given offering */
                  if(node.getAttribute("srsName")) {
